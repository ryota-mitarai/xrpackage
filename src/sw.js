...
const _rewriteUrlToProxy = (u) => {
  if (
    ...
    !/^[a-z]+:\/\/[a-z0-9\-]+\.proxy\.exokit\.org(?:\/|$)/.test(u)
  ) {
...
const _rewriteUrlToRaw = (u) => {
  ...
  o.host =
    match2[2].replace(/--/g, '=').replace(/-/g, '.').replace(/=/g, '-').replace(/\.\./g, '-') +
...
const _proxyHtmlScripts = (htmlString, originalUrl) =>
  htmlString.replace(/(src=["'])([^"']+)(["'])/g, (all, pre, src, post) => {
...
const _rewriteResText = (res, rewriteFn) =>
  res.text().then(
    (text) =>
      new Response(rewriteFn(text) || null, {
        status: res.status,
        headers: res.headers,
      })
  );
...
const _rewriteResExt = (url, originalUrl, headers, res) => {
  if (originalUrl === 'https://aframe.io/releases/0.9.2/aframe.min.js') {
    ...
    return _rewriteResText(
      res,
      (jsString) => 'delete navigator.xr;' // + jsString.replace(
      // /\.getEyeParameters\("left"\);[a-z]+=2\*/g,
      // all => all + '(new FakeXRDisplay().stereo?1:0.5)*'
    ); // );
  } else if (/janusweb\.js$/.test(originalUrl)) {
    return _rewriteResText(res, (jsString) => jsString.replace('https://p.janusvr.com/', ''));
  } else if (originalUrl === 'https://https-moonrider-xyz.proxy.exokit.org/build/build.js') {
    return _rewriteResText(res, (jsString) =>
      jsString.replace(
        'getDistance:function(){var e=this.axis;',
        'getDistance:function(){if (!this.axis)this.axis=[0,0,0];var e=this.axis;'
      )
    );
    ...
    return _rewriteResText(res, (jsString) => {
      const result = jsString
        ...
        .replace(
          'this._btnVR.addEventListener("click",function(){i.isInVRMode?i.exitVR():i.enterVR()})',
          'this._btnVR.addEventListener("click",function(){i.isInVRMode?i.exitVR():i.enterVR()});setTimeout(() => {i.enterVR()})'
        );
      return result;
    });
...
self.addEventListener('message', (e) => {
  ...
  const { method } = data;
  if (method === 'hijack') {
    const { id, startUrl, script, files } = data;
      ...
      clients.get(clientId).then((client) => {
        if (client && client.type === 'window' && client.frameType === 'top-level') {
          if (request.method === 'GET') {
            if (key) {
              return localforage.getItem(key).then((v) =>
                v
                  ? new Response(v)
                  : new Response(null, {
                      status: 404,
              ...
              return localforage.keys().then((v) =>
                v
            ...
            return localforage.removeItem(key).then(() => new Response());
          } else {
